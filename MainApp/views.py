...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        suggestions = Club.objects.exclude(pk__in=followings).exclude(username=request.user.username).order_by("?")[:6]
    return render(request, "uni-zone/index.html", {
        "posts": posts,
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, "uni-zone/login.html", {
...
def register(request):
    if request.method == "POST":
        ...
        try:
            user = Club.objects.create_user(username, email, password)
...
def profile(request, username):
    ...
    return render(request, 'uni-zone/profile.html', {
        "username": user,
        "posts": posts,
...
def comment(request, post_id):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            # Using profanity to check the comment is abusive or not
            print("profanity: ", (profanity.contains_profanity(comment)))
